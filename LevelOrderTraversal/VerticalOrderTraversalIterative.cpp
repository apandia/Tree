...
vector<vector<int>> verticalOrderTraversal(Node* root)
{
    ...
    while (!q.empty())
    {
        ...
        int verticle = q.front().second.first;
        int level = q.front().second.second;

        q.pop();

        m[verticle][level].insert(curr->data);

        if (curr->left)
            q.push({ curr->left, {verticle - 1, level + 1} });

        if (curr->right)
            q.push({ curr->right, {verticle + 1, level + 1} });
    }
...
int main()
{
    ...
    cout << "Vertical Order Traversal (Iterative): " << endl;
